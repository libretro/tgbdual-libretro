...
namespace {
	int GetPrivateProfileInt(const char* opt, const char* key,
							 int def, Resource& rc)
	{
	...
	void GetPrivateProfileString(const char* opt, const char* key,
								 const char* def, char* dst,
								 int len, Resource& rc)
	{
		std::string k = std::string(opt)+":"+key;
		if (rc.has(k)) {
...
setting::setting()
{
...
#ifdef TGB_INI_DIR
	else if (fp = fopen(TGB_INI_DIR "/TGB.ini", "r")) {
		fclose(fp);
		printf("loading %s\n", TGB_INI_DIR "TGB.ini");
		rc.addResourceFile(TGB_INI_DIR "/TGB.ini");
	}
#endif
	else {
	...
	// こんなもんは SDL では関知しない。
/*
	win_pos[0]=GetPrivateProfileInt("win_pos","left",100,rc);
	win_pos[1]=GetPrivateProfileInt("win_pos","top",100,rc);
	win_pos[2]=GetPrivateProfileInt("win_pos","width",(GetSystemMetrics(SM_CXFIXEDFRAME)+1)*2+320,rc);
	win_pos[3]=GetPrivateProfileInt("win_pos","height",(GetSystemMetrics(SM_CYFIXEDFRAME)+GetSystemMetrics(SM_CYMENU)+1)*2+288,rc);
*/
...
/*
	...
	GetPrivateProfileString("directory","media_dir",tmp_save,media_dir,256,rc);
	...
	GetPrivateProfileString("directory","last_load","error",cur_dir,256,rc);
